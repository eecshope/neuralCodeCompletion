...
# what we exactly use
# revise 01/09, add parent hidden states at output
...
outfile = 'output_attention_parent_N.txt'
  ...
  def __init__(self, is_training, config, input_):
    ...
    with tf.variable_scope("RNN"):
      for time_step in range(num_steps):
        ...
        # parent_index = input_.input_dataP[:, time_step]  # retrieval parent hidden state in batch
        # cell_parent = tf.convert_to_tensor([valid_memory[i,-parent_index[i],:] for i in range(batch_size)])
    ...
    zeros_parent = tf.zeros_like(parent, dtype=data_type())

    self.output = tf.reshape(output, [-1, num_steps, size]) #to record the memory for next batch
    wa = tf.get_variable("wa", [size*2+sizeN, size], dtype=data_type())  # feed parent at the output
