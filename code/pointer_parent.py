...
class PTBInput(object):
    ...
    def __next__(self):
        ...
        xn = self._dataN[:, ps: pe]
        yn = self._dataN[:, ps + 1: pe + 1]
        xt = self._dataT_x[:, ps: pe]
        yt = self._dataT_y[:, ps + 1: pe + 1]
        xp = self._dataP[:, ps: pe]
...
class PTBModel(object):
    ...
    def __init__(self, is_training, config, input_):
        ...
        self.eof_indicator = tf.placeholder(tf.int32, [batch_size], name="eof_indicator")
        self.input_dataN = tf.placeholder(tf.int32, [batch_size, num_steps], name="input_dataN")
        self.input_dataP = tf.placeholder(tf.int32, [batch_size, num_steps], name="input_dataP")
        self.targetsN = tf.placeholder(tf.int32, [batch_size, num_steps], name="targetsN")
        self.targetsT = tf.placeholder(tf.int32, [batch_size, num_steps], name="targetsT")
        self.input_dataT = tf.placeholder(tf.int32, [batch_size, num_steps], name="input_dataT")

        with tf.device("/cpu:0"):
            embeddingN = tf.get_variable(
                "embeddingN", [vocab_sizeN, sizeN], dtype=data_type())
            inputsN = tf.nn.embedding_lookup(embeddingN, self.input_dataN)
            inputsP = tf.nn.embedding_lookup(embeddingN, self.input_dataP)
            inputsL = tf.nn.embedding_lookup(embeddingN, self.targetsN)  # target type information

        with tf.device("/cpu:0"):
            embeddingT = tf.get_variable(
                "embeddingT", [vocab_sizeT, sizeT], dtype=data_type())
            inputsT = tf.nn.embedding_lookup(embeddingT, self.input_dataT)
        ...
        labels = tf.reshape(self.targetsT, [-1])
        ...
        new_labels = tf.where(condition_tf, wrong_label,
                              labels)  # only for computing the accuracy, can not be used to compute the loss(cause nan error)
...
def run_epoch(session, model, eval_op=None, verbose=False):
    ...
    for xn, yn, xt, yt, xp, _eof_indicator in tqdm(model.input):
        ...
        feed_dict = {
            ...
            model.input_dataN: xn,
            model.input_dataP: xp,
            model.targetsN: yn,
            model.targetsT: yt,
            model.input_dataT: xt
        }
